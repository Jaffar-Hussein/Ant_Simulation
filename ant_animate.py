...
class Ant:
    def __init__(self, master: tk.Canvas, _id=None, color='black', radius=4, base_pos=0, coords=None, life=[], pv=20,
                 delay=750):
        self.canvas = master
        self.id = _id
        self.color = color
...
class Mainframe(tk.Frame):
    def __init__(self, master, width, height, food_coords, ants_dict, pv, delay=750):
        ...
        for key in self.ants_dict:
            Ant(self.canvas, _id=key, radius=10, color='black', base_pos=self.step_size / 2.,
                coords=self.ants_dict[key]['coords'], life=self.ants_dict[key]['life'], pv=pv, delay=delay)
...
class Visual_App(tk.Tk):
    def __init__(self, ants_dict, foodx, foody, pv, delay=750):
        ...
        self.mainframe = Mainframe(self, width=self.width, height=self.height,
                                   food_coords=self._parse_food(foodx, foody),
                                   ants_dict=self._parse_ants_dict(ants_dict), pv=pv, delay=delay)
    ...
    def _parse_ants_dict(self, ants_dict):
        ...
        for key in ants_dict:
            new_ants_dict[key] = {'coords': list(zip(ants_dict[key]['x'], ants_dict[key]['y'])),
                                  'life': ants_dict[key]['pv']}
